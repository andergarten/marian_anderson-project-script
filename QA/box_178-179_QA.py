...
def get_ws(wb_path):
    ...
    ws = wb["Ms. Coll 200 Box 178179"]
    return ws

def get_file_list(path):
    '''get a list of sorted file name178
    '''
...
def get_ark(ws, folder_name, item_number):
    ...
    for row in ws.iter_rows(min_row = 1, min_col = 7, max_col = 7, max_row = 671):
        for cell in row:
...
def reference_list_rename(path, folder_name, wb_path):
    '''This method is to use reference list to identify referene shot and then rename
    '''
    os.chdir(path)
    # get worksheet
    ws = get_ws(wb_path)
    ...
    print('current reference images list: ' + reference)
...
def reference_size_rename(path, folder_name, wb_path, sample_shot):
    '''This method is to use size to identify reference shot, and then rename
    '''
    os.chdir(path)
    # get worksheet
    ws = get_ws(wb_path)
...
def main():
    ...
    while True:
        ...
        #path = r'/Users/zhiyuzhou/OneDrive/Penn/18_Summer/Work_on-campus/Script/QA/test_example/folder0' + folder_number
        path = drive_letter + r":\Temporary_sceti-completed\MarianAnderson\mscoll200_box" \
               + box_number + "\\folder0" + folder_number
        #print(path)
        ...
        wb_path = drive_letter + r':\Temporary_sceti-completed\MarianAnderson\Student Copy of Marian Anderson Batch 3.xlsx'
        ...
        sample_shot = drive_letter + r":\Temporary_sceti-completed\MarianAnderson\sample_shot.tif"
        #sample_shot = r'/Users/zhiyuzhou/OneDrive/Penn/18_Summer/Work_on-campus/Script/QA/sample_shot.tif'

        # an extra rename step after box 183
        #reference_list_rename(path, folder_number, wb_path)
        reference_size_rename(path, folder_number, wb_path, sample_shot)
        ...
        if os.path.exists(path + ' - Copy') and qa_pass:
